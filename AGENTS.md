...
- **2015:** Full year (Jan–Dec) + ~12 days into Jan 2016
- **2018:** Only **Jan–Aug** (8 months — Sep–Dec data was not provided by ETUFOR)
...
| `validacoes_por_hora` / `validations_per_hour` | **Target variable** — passenger count |
| `hora` | Hour of day (0–23) |
| `d_semana` | Day of week (0=Sunday … 6=Saturday) |
| `d_mes` | Day of month |
| `d_ano` | Day of year |
| `mes` | Month (1–12) |
...
03-cross-year-comparison.ipynb      # Cross-year comparison (2015/2018/2020), stacking ensemble,
                                    # Extreme Gradient Boosting, per-line models
04-results.ipynb                    # Aggregated results visualization, scatter plots

2015/
  05-per-line-models-2015.ipynb     # Per-line models for 2015 (Linear Reg, Ridge, SVR, Decision
                                    # Tree, Random Forest, Bagging, XGBoost, Stacking)
  06-per-line-models-2015-cyclical.ipynb # Same but with sin/cos cyclical feature encoding
  variables.py                      # Holiday & eve-of-holiday definitions per year

other-related-notebooks/
  07-time-series-framing.ipynb      # Time-series framing approach
  08-neural-net-time-series.ipynb   # Neural network with time-series framing
  09-neural-net-standard.ipynb      # Neural network without time-series framing
...
todo.txt                            # Original thesis writing TODOs and experiment notes
```
...
| Neural Network (time-series) | (in other-related-notebooks/) |
...
- **Holiday flags**: Brazilian national + Ceará/Fortaleza local holidays manually defined in `variables.py` for each year.
...
- **Per-line models**: Each bus line gets its own trained model (strategy 1 in the thesis).
